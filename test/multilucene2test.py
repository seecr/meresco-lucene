...
from meresco.lucene.multilucene2 import MultiLucene
...
class MultiLuceneTest(SeecrTestCase):
    ...
    def testAddTypeToSortField(self):
        self.response = JsonDict({
                "total": 887,
                "queryTime": 6,
                "hits": [{"id": "record:1", "score": 0.1234}]
        ...
        consume(self._multiLucene.executeComposedQuery(cq))
        self.assertEqual({
                "sortKeys": [{'core': 'coreA', 'sortBy': 'sortField', 'sortDescending': True, 'type': 'String'}],
                "resultsFrom": "coreA",
