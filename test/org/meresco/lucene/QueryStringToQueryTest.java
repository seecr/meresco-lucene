...
public class QueryStringToQueryTest {
    ...
    public void testComposedQuery() throws Exception {
        JsonObject json = Json.createObjectBuilder()
                .add("composedQuery", Json.createObjectBuilder()
                ...
                .build();
        QueryStringToQuery q = new QueryStringToQuery(new StringReader(json.toString()));
        assertEquals(null, q.query);
        assertEquals("coreA", q.composedQuery.resultsFrom);
        assertEquals(1, q.composedQuery.start);
        assertEquals(10, q.composedQuery.stop);
        assertEquals(new HashSet<String>() {{add("coreA"); add("coreB");}}, q.composedQuery.cores);
        assertEquals(new TermQuery(new Term("field", "value0")), q.composedQuery.queryFor("coreA"));
        assertEquals(new TermQuery(new Term("field", "value1")), q.composedQuery.queryFor("coreB"));
        assertEquals(1, q.composedQuery.sort.getSort().length);
        
        assertEquals("keyA", q.composedQuery.keyName("coreA", "coreB"));
        assertEquals("keyB", q.composedQuery.keyName("coreB", "coreA"));
        assertEquals("keyA", q.composedQuery.keyName("coreA", "coreA"));
        assertEquals("keyB", q.composedQuery.keyName("coreB", "coreB"));
        
        assertArrayEquals(new String[] {"keyA"}, q.composedQuery.keyNames("coreA"));
    }
}
